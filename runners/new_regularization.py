...
from plenoxels_with_GB_feature_length_in_disp32.ops.losses.histogram_loss import interlevel_loss
from plenoxels_with_GB_feature_length_in_disp32.raymarching.ray_samplers import RaySamples
...
class L1motionloss(Regularizer):
    ...
    def _regularize(self, model: LowrankModel, model_out, **kwargs) -> torch.Tensor:
        loss = torch.abs(model_out["rev_disp"] - model_out['pts']).mean()
        return loss
...
class L1volume_rendered_point_loss(): # aug24
    ...
    def _regularize(self, model: LowrankModel, model_out, **kwargs) -> torch.Tensor:
        return torch.abs(model_out["3d_points_rev_grid"]-model_out['pts_rendered']).mean()
